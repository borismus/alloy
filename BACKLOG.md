...
- [ ] Single-parser consolidation — make Rust `config.rs` the only config parser. Add a `GET /api/config` returning the resolved config as JSON (redacting secrets the SPA doesn't need) and route the SPA's config reads/writes (loadConfig, favorites, share toggle, externalEditor, first-run write) through endpoints, removing js-yaml parsing/writing from `src/services/vault.ts`. Keeps the two sides from ever drifting. Depends on the canonical config schema (done).
...
- [ ] UI polish — migrate `src/components/ThinkingDisclosure.tsx` from its custom `useState`/`aria-expanded` button to a React Aria `Disclosure` (Heading + trigger Button + DisclosurePanel), matching the riff change review. Keep the elapsed-time label and the `.panel[hidden]{display:none}` WebKit rule. From the prototype.
- [ ] UI polish (low priority) — introduce a `ui/Tooltip` (React Aria `Tooltip` + `TooltipTrigger`) and replace `title=` attributes on icon-only buttons (header back/close/settings, composer attach/send, model-picker, etc.) for keyboard-accessible, styled tooltips. From the prototype.
