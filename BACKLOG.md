...
-->
- [x] Add a `codex-cli` provider (kind `cli_codex`) to alloy-server, sibling to `cli_claude` (done). Shells out to `codex exec` in a read-only sandbox (`--json`), scrubs `OPENAI_API_KEY` to force ChatGPT/Codex subscription billing, text-only (`supports_tools` false, no MCP bridge), always cloud (never `local`, no private-dir access). Curated model list, config example + docs, and unit tests for prompt flattening + defensive `--json` parsing (7 tests). Also aligned the `/api/models` "local" badge with `provider_is_local` so it can't disagree with private-dir gating.
  - **NEEDS REAL-WORLD VALIDATION**: the exact `codex exec` flags, auth, and `--json` event schema vary by codex version. `base_command` and `parse_agent_message` in `cli_codex.rs` are best-effort and likely need tuning against an installed `codex` binary + live subscription. Streaming is currently one-shot (final message only).
- [ ] Codex follow-ups (after validation): real token streaming from `codex exec --json`, mid-run cancellation (kill the child), image support, and (if feasible) tool access via Codex's own MCP/tooling.
