...
`;
    ...
    // Note capture skill
    const noteCaptureSkillPath = await join(skillsPath, 'note-capture');
    if (!(await exists(noteCaptureSkillPath))) {
      await mkdir(noteCaptureSkillPath, { recursive: true });
      const noteCaptureSkillMd = `---
name: note-capture
...
`;
      await writeTextFile(await join(noteCaptureSkillPath, 'SKILL.md'), noteCaptureSkillMd);
    }

    // Note query skill
    const noteQuerySkillPath = await join(skillsPath, 'note-query');
    if (!(await exists(noteQuerySkillPath))) {
      await mkdir(noteQuerySkillPath, { recursive: true });
      const noteQuerySkillMd = `---
name: note-query
description: Search and retrieve information from your accumulated notes.
---
...
1. Use \`list_directory\` with directory \`notes\` to see what notes exist
2. Read relevant note files based on the query
3. Synthesize an answer with citations to the notes

If you find gaps in the notes, suggest using note-capture to add the missing information.
`;
      await writeTextFile(await join(noteQuerySkillPath, 'SKILL.md'), noteQuerySkillMd);
    }

    // Note linker skill
    const noteLinkerSkillPath = await join(skillsPath, 'note-linker');
    if (!(await exists(noteLinkerSkillPath))) {
      await mkdir(noteLinkerSkillPath, { recursive: true });
      const noteLinkerSkillMd = `---
name: note-linker
